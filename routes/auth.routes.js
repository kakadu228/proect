...
router.post(
    ...
    try {
        ...
        const user = new User({email, flag, activ, code_unt, password: hashPass, two_act})
...
router.post('/save', auth, async (req, res) => {
    try {
        const {flag, coden} =req.body

        const user = await User.findById({_id: req.user.userId})
        console.log(user.code_unt)

        if (coden === user.code_unt){
            User.findByIdAndUpdate(user, { $set: {flag: flag}},
                function(err, doc){
                    doc.code_unt = 'flag'
                })
            res.json({message: 'Сохранено'})
        }else{
            res.json({message: 'Не совпадает'})
        }
...
router.post('/generate_code_3', auth, async (req, res) => {
    try {
        ...
        const message_email = {
            from: 'Контроль веса <kontrol.vesa@bk.ru>',
            to: user.email,
